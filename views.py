...
from Tools.TemplateSetting import TemplateSettingDict
try:
...
def regist():
    ...
    if request.method == 'POST':
        ...
        return render_template('regist.html', **TemplateSettingDict())
...
def login():
    ...
    if request.method == 'POST':
        ...
        return render_template('login.html', **TemplateSettingDict())

# 关于页面
@app.route('/about')
def about():

    return render_template("about.html", **TemplateSettingDict(title="关于Bomd"))

# 个人简历
@app.route('/chenjiaju')
def resume():

    return render_template("resume.html", **TemplateSettingDict(title="陈家驹的简历")) 
...
def zone():
    ...
    if session.has_key('username'):
        ...
        if username == session['username']:
            return render_template("userZone.html", **TemplateSettingDict(user=username, themes=themes))
        else:
            return redirect('/userzone?name=%s' % session['username'])
    else:
        return redirect('/login', **TemplateSettingDict())
...
def Articles():
    ...
    if searchfor:
        ...
        try:
            articles = list(db4essay.listEssays()) # [title, uptime, pwd, body, tag] 时间 上传时间 访问密码 正文 标签
            ...
            return render_template('Articles.html', **TemplateSettingDict(num='all', articles=sub_meeted_articles, title="所有博文", tips=tips, maxPage=maxPage, pageID=pageID, searchfor=searchfor, tag=searchfor))
    # 如果没有标题,说明是显示文章列表
    if not essayTitle:
        try:
            articles = list(db4essay.listEssays())
            maxPage, pageID, articles = pages.pageHandle(articles, 1, pageID)
        except Exception as e:
            return "出错啦!\n%s"%e.message
        else:
            return render_template('Articles.html', **TemplateSettingDict(num='all', articles = articles, title="所有博文", maxPage=maxPage, pageID=pageID, searchfor=searchfor, tag="ALL"))
    # 如果有标题，说明就是某一文章的内容页
    else:
        ...
        try:
            ...
            comments_num = len(comments)                    # 评论数量
        ...
        return render_template('Articles.html', **TemplateSettingDict(num='one', article = Essay, lock = essayLock, title=essayTitle, tag="ARTICLES", comments=comments, comments_num=comments_num, nickname=nickname, email=email))
...
def Resources():
    ...
    return render_template('Resources.html', **TemplateSettingDict(resources = resources))
...
def comment():
    ...
    if not username or not email or not content:
        return "不要空哦"
